...
public class GptService {
    ...
    public String generateResponse(String inputJson, String inputText) {
        String prompt = String.format("""
            ...
            6.  입력된 텍스트에서 출장금액은 모든 금액을 추출하여 소수점, 반올림, 단위 변환(천, 만 등)은 하지 말고 오직 "원 단위의 정수"만 더하세요.\s
                    숫자를 추출할 때는 반드시 "원"이 붙은 항목을 그대로 사용하세요. \s
                    예: "500원" → 500, "12,300원" → 12300, "12673KRW" → 12673\s
                    반드시 "원" 단위까지 정수로 변환하여 합산해야 하며 1000원, 500원, 100원, 50원 같은 작은 단위도 절대 생략하지 마세요. \s
                    마지막으로 한번 더 출장금액이 모든 추출된 텍스트의 모든 금액을 더한 값인지 확인을 하고 아니라면 다시 모든 값을 더한 뒤에 출장금액 값을 넣으세요.
            ...
            4.  출장금액의 값은 절대로 임의로 정하지 않고 식비정산금, 숙박비금액, 금액1, 금액2, 금액3, 금액4을 모두 합친 값으로 채우세요.
